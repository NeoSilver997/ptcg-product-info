...
class EventDataImporter:
    ...
    def _insert_deck_card(self, deck_id, card):
        ...
        # Insert card - since decks are only imported once, this should work fine
        # If the same card appears multiple times in JSON (shouldn't happen), combine quantities
        cursor.execute("""
        ...
        # If card was ignored (duplicate in same deck), combine quantities
        if cursor.rowcount == 0:
            cursor.execute("""
                UPDATE deck_cards 
